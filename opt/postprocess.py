...
utilfile = os.path.join(os.getcwd(), os.pardir, 'CMakeLists.txt')
...
with open(cmake_src_path) as f:
    for line in f:
        if "set (MapServer_VERSION_MAJOR" in line:
            ms_major = line.rstrip()
            ms_major_get = True
        elif "set (MapServer_VERSION_MINOR" in line:
            ms_minor = line.rstrip()
            sm_minor_get = True
        elif "set (MapServer_VERSION_REVISION" in line:
            ms_patch = line.rstrip()
            ms_patch_get = True

        if ms_major_get and ms_minor_get and ms_patch_get:
            break
